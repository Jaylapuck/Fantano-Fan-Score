...
url = "https://www.youtube.com/watch?v=VzHT35kxLuQ&list=PLP4CSgl7K7ori6-Iz-AWcX561iWCfapt_"

driver.get(url)

description_class = 'style-scope ytd-video-secondary-info-renderer'

description_box = driver.find_element_by_xpath("//div[@class='style-scope ytd-video-secondary-info-renderer']")

driver.find_element_by_xpath("//yt-formatted-string[@class='more-button style-scope ytd-video-secondary-info-renderer']").click()

footer = [i.text for i in driver.find_elements_by_xpath("//div[@id='description']//span[@class='style-scope yt-formatted-string']")][-1]
...
def extract_data(driver):
    ...
    footer = [i.text for i in driver.find_elements_by_xpath(
        "//div[@id='description']//span[@class='style-scope yt-formatted-string']")][-1]
